...
# ================================================
pheno_raw <- read_excel("data/DatasetGWAS.xlsx", sheet = 2,
                        n_max = pheno_data_rows,
                        col_names = TRUE)

# Your pheno columns look like:
# Phenotype | Allele.A | Allele.B | R1 | R10 | R100 ...
# Keep only the trait name column and individual columns (drop Allele.A, Allele.B)
...
is_letter_row <- apply(pheno_raw[, -1], 1, function(x) {
  any(grepl("[A-Za-z]", na.omit(x)))
})
...
# ================================================
geno_raw <- read_excel("data/DatasetGWAS.xlsx", sheet = 2,
                       skip = geno_header_row - 1,
                       col_names = TRUE)

# Grab individual names from the pheno header row in the raw sheet
# The pheno header is row 1 of the raw sheet, columns start after 
# Phenotype | ...2 | ...3 (the first 3 columns)
...
# Format chromosome as chr01, chr02 etc.
geno$Chromosome <- paste0("chr", sprintf("%02d", as.integer(geno$Chromosome)))
?paste0
# Remove UNMAPPED or NA chromosome markers
geno <- geno[!is.na(geno$Chromosome), ]
