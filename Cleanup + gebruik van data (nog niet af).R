...
# individuals become rows, traits become columns (dat moet)
...
#geno <- geno[missing_rate < 0.10, ]   # kan een issue zijn dus ik comment dit
...
for(tr in Traits) {
  name <- paste0("Plots/QQ//QQplot_", tr, ".png")
...
print(data2@threshold)
######################################################
ggsave(
  filename = paste0("Plots/Manhattan/ManhattanPlot_All.png"),
...
for(tr in Traits) {
  name <- paste0("Plots/Manhattan/ManhattanPlot_", tr, ".png")
...
# voor als een chromosoom uitstaat
######################################################
ggsave(
  filename = paste0("Plots/LD/LD_Plot_All.png"),
